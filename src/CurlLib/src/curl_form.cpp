...
void curl_form::copy_ptr(struct curl_httppost **ptr, const struct curl_httppost *old_ptr) {
    if (old_ptr->name) {
        ...
        strcpy_s((*ptr)->name,old_ptr->namelength*sizeof(char),old_ptr->name);
        // Copy the namelength
    ...
    if (old_ptr->buffer) {
        ...
        strcpy_s((*ptr)->buffer,old_ptr->bufferlength*sizeof(char),old_ptr->buffer);
    ...
    if (old_ptr->contents) {
		...
		strcpy_s((*ptr)->contents,old_ptr->contentslength*sizeof(char),old_ptr->contents);
    ...
    if (old_ptr->contenttype) {
		...
		strcpy_s((*ptr)->contenttype,strlen(old_ptr->contenttype)*sizeof(char),old_ptr->contenttype);
    }
    if (old_ptr->showfilename) {
        // Copy file name
        this->is_null((*ptr)->showfilename = (char *)malloc(strlen(old_ptr->showfilename)*sizeof(char)));
		strcpy_s((*ptr)->showfilename,strlen(old_ptr->showfilename)*sizeof(char),old_ptr->showfilename);
    }
    // Copy flags
